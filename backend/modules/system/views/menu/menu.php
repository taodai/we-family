...
<body>
    <table id="dg" class="easyui-treegrid"
            data-options="  
                ...
                fit:true,
                toolbar:'#toolbar',
                url:'<?php echo Url::home()?>/system/menu/list'
            ">  
        <thead>  
            <tr>
...
<script>
    ...
    function add()
    {
        url = "<?php echo Url::home()?>/system/menu/menu-add";
        formClear();
    ...
    function medit()
    {
        ...
        if (row){  
            ...
            // 加载数值
            $('#fm').form('load',row);  
            url = "<?php echo Url::home()?>/system/menu/menu-edit?sm_id="+row.sm_id;
        }else{
            $.messager.show({  
                title: '错误提示',  
                msg: '请先选择一条记录'  
