...
class HaqileWechat
{
	...
	const FILE_URL_PREFIX = 'http://file.api.weixin.qq.com/cgi-bin';
	...
	private function checkSignature()
	{
		...
		$tmpStr = sha1( $tmpStr );
		// $fp=fopen('log.txt',"a+");
		// fwrite($fp,$signature."/////".$timestamp."/////".$nonce."/////".$token."\n".$_GET['echostr']."\n".$tmpStr."\n");
		// fclose($fp);
		if( $tmpStr == $signature ){
			return true;
		}else{
...
//    }
	...
	*/
	...
	public function imagetext($newsData=array())
	{
		$textTpltitle="<xml>
			...
			$time=time();
			$msgType = "image";
            $resultStr = sprintf($textTpl, $this->getRevFrom(), $this->getRevTo(), $time, $msgType, $mediaId);
            echo $resultStr;
		
	}
	public function pic($mediaId)
	{
		$textTpl="<xml>
			<ToUserName><![CDATA[%s]]></ToUserName>
			<FromUserName><![CDATA[%s]]></FromUserName>
...
//</xml>
	 ...
	 * 上传媒体文件
	 */
	public function uploadMedia($url){
		
			
		$filedata = array("file1"  => "@".$url);
		
		$resu = $this->https_request('http://file.api.weixin.qq.com/cgi-bin'.'/media/upload?'.'access_token='.$this->access_token.'&type=image',$filedata);
		if ($resu)
		{
			$json = json_decode($resu,true);
			return $json['media_id'];
			//$weObj->text($resu)->reply();
		}else{
			return false;
			//$weObj->text("false")->reply();
		}
	}
	/**
	 * 创建二维码ticket
